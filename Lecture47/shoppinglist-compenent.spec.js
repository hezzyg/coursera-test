describe('shoppingList component', function() {
	...
	it('should detect no cookies in its list', function() {
		//Pass bindings that are needed for thr test
		var bindings = {items: [{name: 'item1', quantity: '1'}]};
		var ctrl = $componentController('shoppingList', {$element: null}, bindings);

		var coockiesInList = ctrl.cookiesInList();
		expect(coockiesInList).toEqual(false);
	});

	it('should detect cookies in its list', function() {
		//Pass bindings that are needed for thr test
		var bindings = {items: [{name: 'cookies', quantity: '1'}]};
		var ctrl = $componentController('shoppingList', {$element: null}, bindings);

		var coockiesInList = ctrl.cookiesInList();
		expect(coockiesInList).toEqual(true);
	});
